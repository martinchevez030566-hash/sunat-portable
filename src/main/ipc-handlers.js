...
const { createCompany, getAllActiveCompanies, updateActiveCompanyId, getActiveCompanyId, toggleCompanyActive } = require('./services/company-service');
...
function registerIpcHandlers() {
  ...
  // 🔹 NUEVO: Selector seguro de imagen (main process)
  ipcMain.handle('dialog:select-image', async () => {
    try {
      ...
      if (buffer.length > 500 * 1024) return { success: false, error: 'La imagen es muy grande (máx 500KB)' };
