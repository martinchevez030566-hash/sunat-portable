...
function registerIpcHandlers() {
  ...
  ipcMain.handle('db:run-bootstrap', async () => {
    try { return await runBootstrap(); } catch (e) { return { success: false, error: e.message }; }
  });
  ...
  // 🔍 settings:get con lectura CORRECTA
  ipcMain.handle('settings:get', async (event, key) => {
    await initDatabase();
    const res = queryRow('SELECT value FROM app_settings WHERE key = ?', [key]);
    const val = res?.value ?? null;
    console.log(`🔍 [IPC] settings:get("${key}") → "${val}"`);
    return val;
  });

  // ✅ settings:update con escritura directa y verificación
  ipcMain.handle('settings:update', async (event, { key, value }) => {
    await initDatabase();
    console.log(`📝 [IPC] settings:update("${key}", "${value}") → Iniciando`);
    // Usamos dbInstance.run() directamente para mayor fiabilidad
    dbInstance.run("INSERT OR REPLACE INTO app_settings (key, value, updated_at) VALUES (?, ?, datetime('now'))", [key, String(value)]);
    saveDatabase();
    const verify = queryRow('SELECT value FROM app_settings WHERE key = ?', [key]);
    console.log(`✅ [IPC] settings:update("${key}") → Verificado